...
- **Surface a real finding inline; never bury it in the log.** A blocker — or a genuinely actionable concern a human should act on or reply to on a specific line — goes as an **inline review comment anchored to that line**, where it can be threaded and resolved. Do NOT demote such a concern to a run-note / "new observation" on the log surface, and do NOT fold it into the top-level summary (top-level comments can't be threaded or resolved). The log surface is for _what you traced_, not for _findings you declined to surface_. The dividing line from a suppressed nit is actionability on a line in this diff: if you'd want a human to act on or reply to it, post it inline; if it's speculative or out-of-diff, omit it per "What is NOT a blocker." A `no blockers` summary MUST be consistent with the inline comments you actually left — never report "no blockers" while a real concern sits only in the run-notes.
