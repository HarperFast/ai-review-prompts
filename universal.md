...
- **Suggestions follow the same posting contract as findings — but clearly non-gating.** Where the workflow supports inline review comments (the agentic Claude leg), post each suggestion as an **inline comment anchored to its line**, prefixed `Suggestion (non-blocking):` — a resolvable, replyable thread the author ticks off or dismisses, exactly like a finding but explicitly optional. Where inline isn't available (the single-shot Gemini leg), collect them in one top-level `### Suggestions (non-blocking)` section, one bullet each (`path:line — improvement (benefit)`). Either way: clearly optional, never in the blocker list, never a `REQUEST_CHANGES`.
- **PR comments stay concise** — every reader pays the cost. If zero blockers and no qualifying suggestion, the PR comment is **one sentence** (e.g. `Reviewed; no blockers found.`). If zero blockers but you have ≥1 qualifying Suggestion, keep the top-level comment to that one sentence and post the suggestions per the bullet above. The "what I traced" calibration summary — one line per surface verified, no full re-derivation — belongs in the workflow's **log surface** (a per-PR issue threaded by a `Log review to <log-repo>` step) when one is wired. Workflows without a log surface MAY keep the tracing on the PR as a calibration aid, but only until a log surface is added.
