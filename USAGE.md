...
- The variable is a **repo setting, not code** — flip a reviewer between always-on and opt-in without a PR or a pin bump, and independently of the `uses:` pin. The caller's job `if:` reads it, identically for both reviewers (only the variable and label names differ): the provider's own label (`labeled` + `github.event.label.name`), any non-draft, trusted-author non-label event under `<REVIEWER>_ALWAYS_ON == 'true'`, or a `ready_for_review` flip on a PR that carries the opt-in label. The caller **names the label** — an unrelated label must neither start the job nor share the reviewer's cancelling concurrency group (GitHub applies `cancel-in-progress` at queue time, *before* any job `if:` runs, so an unscoped group lets an ineligible event cancel an in-flight review and then skip: a silently lost review). WHO is admitted remains the reusable `authorize` job's decision. Thin callers set **no `concurrency` block**: cancellation is owned by the reusable's *review job* (a job-level group that engages only after the authorize, skip, and key gates pass), so an event that will not produce a review — an unauthorized actor, an unrelated label, a skipped mechanical diff, or a **stale event whose head the PR has moved past** — can never cancel a legitimate in-flight review; a newer *authorized* run supersedes an older one. Stale events are refused admission, and a run that discovers it was superseded after acquiring the slot fails itself rather than reviewing a stale commit — narrowing the reorder window to the gap between authorize's final freshness read and the review job's queue entry; in the residual case (a push landing in that gap) the tip is left unreviewed until its next event, visibly (cancelled/failed checks on the old head; ai-review-prompts#92 tracks closing it fully). The standalone example (`examples/claude-review.yml`), whose own gate is the authorization, instead carries an eligibility-scoped workflow-level group. `tests/caller-concurrency-contract.test.sh` pins both patterns. **Migrating a caller: remove its workflow-level `concurrency` block in the same PR as the pin bump** — a retained legacy group keeps cancelling at workflow-queue time (before any gate) and preserves exactly the residual this design removes; even a bare `group:` without `cancel-in-progress` replaces a pending run in the same group.
