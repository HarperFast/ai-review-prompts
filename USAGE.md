...
- The variable is a **repo setting, not code** — flip a reviewer between always-on and opt-in without a PR or a pin bump, and independently of the `uses:` pin. The caller's job `if:` reads it, identically for both reviewers (only the variable and label names differ): the provider's own label (`labeled` + `github.event.label.name`), any non-draft, trusted-author non-label event under `<REVIEWER>_ALWAYS_ON == 'true'`, or a `ready_for_review` flip on a PR that carries the opt-in label. The caller **names the label** — an unrelated label must neither start the job nor share the reviewer's cancelling concurrency group (GitHub applies `cancel-in-progress` at queue time, *before* any job `if:` runs, so an unscoped group lets an ineligible event cancel an in-flight review and then skip: a silently lost review). WHO is admitted remains the reusable `authorize` job's decision. The caller's `concurrency.group` is **eligibility-scoped**: only events that will actually produce a review share the cancelling group, and `labeled` / `ready_for_review` events run *without* cancelling (their full authorization is decided downstream; a rare duplicate review is strictly cheaper than a lost one). The label name now lives in both the caller and the reusable — `tests/caller-concurrency-contract.test.sh` pins the whole event matrix, so an edit that drifts trips a test. See `examples/claude-review.yml` for the canonical shape.
