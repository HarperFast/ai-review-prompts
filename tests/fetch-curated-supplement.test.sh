...
if [[ "$URL" == *"/comments?"* ]]; then
	...
	printf '%s\n' '[{"number":99,"title":"False negatives 2026-08-17","body":"boilerplate","labels":[{"name":"false-negatives"}]}]'
fi
STUB
chmod +x "$TMP/bin/gh"
...
run_fetch complete
assert_eq "$(jq 'length' "$TMP/supplement.json")" "1" "matching curated issue is retained"
